...
$Actualizacion = "UPDATE tf_manzanas SET codi_mzna='$dg_manzana', nume_mzna='$upc_mzna' WHERE id_mzna='$IDManzana'";
...
$Actualizacion = "UPDATE tf_lotes SET codi_lote='$dg_lote', id_hab_urba='$upc_codhu', mzna_dist='$upc_mzna', ".
                "lote_dist='$upc_lote', sub_lote_dist='$upc_sublote', estructuracion='$dp_estructura', ".
                "zonificacion='$dp_zonifica', cuc='$dg_cuc8', zona_dist='$upc_zse' WHERE id_lote='$IDLote'";
...
$Actualizacion = "UPDATE tf_edificaciones SET codi_edificacion='$dg_edificacion', tipo_edificacion='$upc_cmb_tipoedi', ".
                "nomb_edificacion='$upc_nomedi', clasificacion='$dp_cmb_claspre' WHERE id_edificacion='$IDEdificacion'";
...
//echo '-'.$upc_cmb_tipoint.'-';
$Seleccion = "SELECT id_uni_cat FROM tf_uni_cat WHERE id_uni_cat='$IDUniCat'";
$Actualizacion = "UPDATE tf_uni_cat SET codi_entrada='$dg_entrada', codi_piso='$dg_piso', codi_unidad='$dg_unidad', ".
                "tipo_interior='$upc_cmb_tipoint', cuc='$cuc', codi_hoja_catastral='$dg_hojacatastral', ".
                "codi_pred_rentas='$dg_codpredial', nume_interior='$upc_numint', unid_acum_rentas='$dg_unicodpredial', ".
                "codi_cont_rentas='$dg_codcontribuyente' ".
                "WHERE id_uni_cat='$IDUniCat'";
...
if ($dp_medcam_fre != '' || $dp_medsegtitu_fre != '' || $dp_colcam_fre != '' || $dp_colsegtitu_fre != '' ||
    ...
    $dp_medcam_fre = substr($dp_medcam_fre, 0, 20);

    $dp_medcam_der = substr($dp_medcam_der, 0, 20);

    $dp_medcam_izq = substr($dp_medcam_izq, 0, 20);

    $dp_medcam_fon = substr($dp_medcam_fon, 0, 20);


    $dp_medsegtitu_fre = substr($dp_medsegtitu_fre, 0, 20);

    $dp_medsegtitu_der = substr($dp_medsegtitu_der, 0, 20);

    $dp_medsegtitu_izq = substr($dp_medsegtitu_izq, 0, 20);

    $dp_medsegtitu_fon = substr($dp_medsegtitu_fon, 0, 20);


    $dp_colcam_fre = substr($dp_colcam_fre, 0, 20);

    $dp_colcam_der = substr($dp_colcam_der, 0, 20);

    $dp_colcam_izq = substr($dp_colcam_izq, 0, 20);

    $dp_colcam_fon = substr($dp_colcam_fon, 0, 20);


    $dp_colsegtitu_fre = substr($dp_colsegtitu_fre, 0, 20);

    $dp_colsegtitu_der = substr($dp_colsegtitu_der, 0, 20);

    $dp_colsegtitu_izq = substr($dp_colsegtitu_izq, 0, 20);

    $dp_colsegtitu_fon = substr($dp_colsegtitu_fon, 0, 20);
...
if ($ct_cmb_condtitu != '05') {
    ...
    if ($itc_cmb_tipotitu == '1') {
        ...
        $Modifica = "UPDATE tf_personas SET nombres='$itc_nombre1' , ape_paterno='$itc_paterno1', ".
                "ape_materno='$itc_materno1' ".
        ...
        if ($itc_numdoc2 != '') {
            ...
            $Modifica = "UPDATE tf_personas SET nombres='$itc_nombre2' , ape_paterno='$itc_paterno2', ".
                        "ape_materno='$itc_materno2' ".
                        "WHERE id_persona='$IDPersona2'";
    ...
    echo "<meta http-equiv='refresh' content='1;url=http://".$_SERVER['HTTP_HOST']."/syslevantamiento/fichaIndividual/edit_individual.php?id=".$IDFicha."'/>";
