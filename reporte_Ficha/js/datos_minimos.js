	...
	{	
        ...
        if(codSector.value == '' || codSector.value.length<2 || (codSector.value == 0))
			{ alert('Ingrese codigo de Sector! - 2 digitos diferentes de cero');		sw=1;	codSector.focus();	return false;}
		if(codManzana.value == '' || codManzana.value.length<3 || (codManzana.value == 0))
			{ alert('Ingrese codigo de Manzana! - 3 digitos diferentes de cero'); 		sw=1;	codManzana.focus();	return false;}
		if(codLote.value == '' || codLote.value.length<3 || (codLote.value == 0))
			{ alert('Ingrese codigo de Lote! - 3 digitos diferentes de cero');			sw=1;	codLote.focus();	return false;}
		if(codEdifica.value == '' || codEdifica.value.length<2 || (codEdifica.value == 0))
			{ alert('Ingrese codigo de Edificacion! - 2 digitos diferentes de cero');	sw=1;	codEdifica.focus();	return false;}
		if(codEntrada.value == '' || codEntrada.value.length<2 || (codEntrada.value == 0))
			{ alert('Ingrese codigo de Entrada! - 2 digitos diferentes de cero');		sw=1;	valores.codEntrada.focus();	return false;}
		if(codPiso.value == '' || codPiso.value.length<2 || (codPiso.value== 0))
			{ alert('Ingrese codigo de Piso! - 2 digitos diferentes de cero');			sw=1;	codPiso.focus();	return false;}
		if(codUnidad.value == '' || codUnidad.value.length<3 || (codUnidad.value == 0))
			{ alert('Ingrese codigo de Unidad! - 3 digitos diferentes de cero');		sw=1;	codUnidad.focus();	return false;}

        /************************* Ubicacion del Predio *************************/
        if(idEstado.value == '' || idEstado.value == 0)
		{	alert('Elija un Tipo de estado');					sw=1;	idEstado.focus();	return false;	}
		if(urbanAuthorizationName.value == '' )
		{	alert('Elija el Nombre de la Habilitacion');		sw=1;	urbanAuthorizationName.focus();	return false;	}
        if(grupoHU.value == '' || grupoHU.value == 0)
		{	alert('Elija un Grupo de Habilitacion');			sw=1;	grupoHU.focus();	return false;	}
        if(mznaDist.value == '' || mznaDist.value == 0)
		{	alert('Ingrese una manzana');						sw=1;	mznaDist.focus();	return false;	}
        if(loteDist.value == '' || loteDist.value == 0)
		{	alert('Ingrese un lote');							sw=1;	loteDist.focus();	return false;	}

        /***************************** Descripcion del predio ***********************/
        if(useAuthorizationName.value == '')
		{	alert('Elija el uso para el predio!');					sw=1;	useAuthorizationName.focus();	return false;	}
		if(useReferencial.value == '' || useReferencial.value == 0)
		{	alert('Ingrese una Referencial Uso');						sw=1;	useReferencial.focus();	return false;	}

        /******************************* Construcciones ***************************/
        if(nroPiso.value == '' || nroPiso.value == 0)
		{	alert('Elija un Nùmero de Piso');							sw=1;	nroPiso.focus();	return false;	}
        if(codWallandColumns.value == '' || codWallandColumns.value == 0)
		{	alert('Elija un Tipo de categoria para Muros y Columnas');	sw=1;	codWallandColumns.focus();	return false;	}
        if(codCeiling.value == '' || codCeiling.value == 0)
		{	alert('Elija un Tipo de categoria para Techos');			sw=1;	codCeiling.focus();	return false;	}
        if(codFloors.value == '' || codFloors.value == 0)
		{	alert('Elija un Tipo de categoria para Pisos');				sw=1;	codFloors.focus();	return false;	}
        if(codDoorandWindow.value == '' || codDoorandWindow.value == 0)
		{	alert('Elija un Tipo de categoria para Puertas y Ventanas');sw=1;	codDoorandWindow.focus();	return false;	}
        if(codMaterial.value == '' || codMaterial.value == 0)
		{	alert('Elija un Tipo de material');							sw=1;	codMaterial.focus();	return false;	}
		...
		{	alert('Elija la Fecha de Levantamiento');					sw=1;	fechaTecnico.focus();	return false;	}
