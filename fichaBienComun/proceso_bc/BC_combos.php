...
{	$BD=new BaseDeDato(SERVIDOR,PUERTO,BD,USUARIO,CLAVE);
	...
	{ 	case 1:
		...
		if ($num==14){
		$Consulta="SELECT codi_uso, codi_uso ||' - '||  desc_uso FROM tf_usos_bc ORDER BY codi_uso ASC"; 
	...
	$consulta_combo = $BD->Consultas($Consulta);
		...
		elseif ($num==21){
		$Consulta="SELECT id_persona, nume_doc ||' - '|| nombres ||' '|| ape_paterno ||' '|| ape_materno AS apenom FROM tf_personas WHERE tipo_funcion='2' ORDER BY ape_paterno, ape_materno, nombres ASC"; 
	$consulta_combo = $BD->Consultas($Consulta);
		}
		elseif ($num==22){
		$Consulta="SELECT id_persona, nume_doc ||' - '|| nombres ||' '|| ape_paterno ||' '|| ape_materno AS apenom FROM tf_personas WHERE tipo_funcion='3' ORDER BY ape_paterno, ape_materno, nombres ASC"; 
	$consulta_combo = $BD->Consultas($Consulta);
		}
		elseif ($num==23){
		$Consulta="SELECT id_persona, nume_doc ||' - '|| nombres ||' '|| ape_paterno ||' '|| ape_materno AS apenom FROM tf_personas WHERE tipo_funcion='4' ORDER BY ape_paterno, ape_materno, nombres ASC"; 
	$consulta_combo = $BD->Consultas($Consulta);
		}
		else{
		//relizamos consulta según las variables
		$Consulta="SELECT codigo, codigo ||' - '||  desc_codigo FROM tf_tablas, tf_tablas_codigos WHERE tf_tablas.id_tabla = tf_tablas_codigos.id_tabla AND tf_tablas.id_tabla = '$iden'"; 
	$consulta_combo = $BD->Consultas($Consulta);
		}
