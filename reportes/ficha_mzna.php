...
include '../configuracion/constantes.php'; 
    ...
    $query_lotes="SELECT    CodCata,PR_Predio_Contribuyente.NroFichaInspeccion as NumFicha, 
          PR_Predio.CodPredio as CodPredio,
          PR_Predio.CodUrba,
          Manzana,
          Lote,
          PR_Predio.CodCalle,
          ...
          AreaTerreno,URBANIZACION.nombre as NombreUrb, VIA.nombvia as Nombrevia
          FROM PR_Predio,PR_Predio_Contribuyente,PR_Persona,URBANIZACION,VIA
          WHERE (CodCata  LIKE '$CodCata') AND 
    ...
    $result = sqlsrv_query($conn,$query_lotes);
 ...
 while( $data_lotes = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC) ) 
{   

          $CodCata = $data_lotes['CodCata'];   
          $CodPredio=$data_lotes['CodPredio'];  
          $NumFicha  = $data_lotes['NumFicha'];
   
                       

      $sql = "SELECT CodCata,
                PR_Predio.CodPredio,PR_Predio.CodUrba,PR_Predio.CodCalle,NumFinca,
                Manzana,Lote,Residencial,Edificio,Piso,Interior,
                TipoPredio,OrigenPredio,AreaTerreno,AreaOcupada,
                CondicionPredio,FrenteAParque,
                PR_Predio_Contribuyente.NroFichaInspeccion as NumFicha,
                URBANIZACION.nombre as NombreUrb, VIA.nombvia as Nombrevia

        FROM    PR_Predio,PR_Predio_Contribuyente,URBANIZACION,VIA
        WHERE (PR_Predio.CodPredio = '$CodPredio') AND  
              (PR_Predio_Contribuyente.NroFichaInspeccion='$NumFicha') AND
              (PR_Predio.CodUrba=URBANIZACION.codurba)AND
              (PR_Predio.CodCalle=VIA.codcalle)";  
...
<body>
          ...
          <table>
              <tbody>
                  ...
                  <tr>
                      <td align="left">COD. CATA : <?php echo $data_ubicacion['CodCata']?></td>
                      <td align="left" colspan="2">COD. PREDIO : <?php echo $CodPredio=$data_ubicacion['CodPredio']?></td>
    ...
    WHERE   PR_Predio_Contribuyente.CodPredio = '$CodPredio' 
            AND PR_Predio_Contribuyente.CodContribuyente=PR_Persona.CodPersona
            AND NroFichaInspeccion='$NumFicha'";  
